...
const ukeData = computed<UkeRow[]>(() => {
  const lines = ukeCsvRaw.split(/\r?\n/).filter((line) => line.trim().length > 0)
...
const ukeCards = computed(() => ukeData.value)
const totalTechniques = computed(() => ukeCards.value.length)
...
const currentCard = computed(() => {
  if (ukeCards.value.length === 0) {
    return null
  }
  return ukeCards.value[currentIndex.value]
})
...
const showNextCard = () => {
  ...
  if (mode.value === 'list') {
    currentIndex.value = (currentIndex.value + 1) % ukeCards.value.length
  } else {
    if (ukeCards.value.length === 1) {
      currentIndex.value = 0
    } else {
      let nextIndex = currentIndex.value
      while (nextIndex === currentIndex.value) {
        nextIndex = Math.floor(Math.random() * ukeCards.value.length)
      }
      currentIndex.value = nextIndex
    }
  }
  techniqueNumber.value = (techniqueNumber.value % ukeCards.value.length) + 1

  ...
  <div class="emblem-view">
    <div class="header">
      ...
      <div class="title-container">
        <h2 class="view-title">{{ t('home.cards.emblem.title') }}</h2>
        <span class="view-subtitle">{{ t('home.cards.emblem.desc') }}</span>
      </div>
    </div>
    <div class="controls">
...
import emblemCsvRaw from '../../emblem.csv?raw'

const { t } = useI18n({ useScope: 'global' })
const router = useRouter()

const emblemImages = import.meta.glob('../assets/emblem/*.webp', { eager: true, query: '?url', import: 'default' })

const parseEmblem = (csvRaw: string) => {
  ...
  return lines.slice(1).map((line, index) => {
    ...
    const imagePath = Object.keys(emblemImages).find((path) => path.endsWith(`/${position}.webp`))

    return {
      position,
      caption,
      alt: description,
      src: imagePath ? (emblemImages[imagePath] as string) : null,
    }
  })
}
...
const emblem = parseEmblem(emblemCsvRaw)
const csvForSession = ['position,name,description', ...emblem.map((item) => `${item.position},${item.caption},${item.alt}`)].join('\n')
...
const currentCard = computed<EmblemCard | null>(() => {
  const rawCard = rawCurrentCard.value
  if (!rawCard) {
    return null
  }

  const found = emblem.find((item) => item.position === rawCard.position)
  return found ?? null
})

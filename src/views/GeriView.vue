  ...
  <div class="geri-view">
    <div class="header">
      ...
      <div class="title-container">
        <h2 class="view-title">Geri Wasa</h2>
        <span class="view-subtitle">Kicking Techniques</span>
      </div>
    </div>
    <div class="controls">
...
import geriCsvRaw from '../../geri.csv?raw'
...
const geriData = computed<GeriRow[]>(() => {
  const lines = geriCsvRaw.split(/\r?\n/).filter((line) => line.trim().length > 0)
...
const geriCards = computed(() => geriData.value)
const totalTechniques = computed(() => geriCards.value.length)
...
const currentCard = computed(() => {
  if (geriCards.value.length === 0) {
    return null
  }
  return geriCards.value[currentIndex.value]
})
...
const showNextCard = () => {
  ...
  if (mode.value === 'list') {
    currentIndex.value = (currentIndex.value + 1) % geriCards.value.length
  } else {
    if (geriCards.value.length === 1) {
      currentIndex.value = 0
    } else {
      let nextIndex = currentIndex.value
      while (nextIndex === currentIndex.value) {
        nextIndex = Math.floor(Math.random() * geriCards.value.length)
      }
      currentIndex.value = nextIndex
    }
  }
  techniqueNumber.value = (techniqueNumber.value % geriCards.value.length) + 1
